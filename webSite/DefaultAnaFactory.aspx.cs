...
public partial class DefaultAnaFactory : System.Web.UI.Page
{
    ...
    private void bindData()
    {
        ...
        String sqlstring = String.Format(@"select area,factory,sum(checkposition) as checkposition,
sum(passPosition) as passPosition,FactoryNo as AreaNo from vw_jobmain2 where area='{2}' and jobdate between '{0}' and '{1}' 
group by area, factory,factoryNo", txtBegin.Text, d2.AddDays(1).ToString("yyyy-MM-dd"),f);
        ...
        Chart3.Series[0].Points.DataBindXY(dv, "factory", dv, "checkPosition");
        Chart3.Series[1].Points.DataBindXY(dv, "factory", dv, "passPosition");
        Chart3.Series[2].Points.DataBindXY(dv, "factory", dv, "AreaNo");
