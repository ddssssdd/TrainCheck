...
namespace TrainCheck
{
    public class JobMain
    {
        ...
        public JobDetail findByBarCode(string barcode) 
        {
            foreach (JobDetail detail in Items)
            {
    ...
    public class Lookup
    {
        public static string Description(string barcode)
        {
            string result = "";
            string lineType = "客车线";
            string indexNo = "";
            ...
            {
                is16 = true;
                if (barcode.Substring(8, 1) == "2")
                {
                    lineType = "货车线";
                }

                indexNo = barcode.Substring(9, 4);
                if (int.Parse(indexNo) % 2 != 0)
                {
                    upDown = "下行";
                }
            ...
            if (is16)
            {
                return String.Format("{0},{1}:第{2},{3}", result, lineType, indexNo, upDown);
            }
            else
            {
                return string.Format("{0},第{1}", result, indexNo);
            }
